...
export const WidgetLayout = (props) => {
  ...
  return (
    <AnimatePresence>
      {toggleWidget && (
        <motion.div
          className={`fixed bottom-5 ml-5 ${position}-5 z-50 flex flex-col rounded-[1.8rem] bg-white font-lato  ring-1 ring-black/5  sm:h-[calc(50%+200px)] sm:w-[calc(50%)] md:h-[calc(50%+100px)] md:w-[calc(50%)] lg:h-[calc(100%-100px)]  lg:w-[calc(25%)] xs:-mr-5   xs:${position}-0 xs:h-[calc(100%-200px)] xs:w-full`}
          animate={{ y: -60 }}
          exit={{ opacity: 0 }}
